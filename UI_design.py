...
class ActorCriticApp(QMainWindow,):
    ...
    def calculate_state_evolution(self):
        ...
        try:
            ...
            # 颜色定义
            colors = ['#90EE90', '#5A9BD5', '#2196F3', '#0D47A1', '#000000']
    ...
    def run_prediction(self):
        ...
        try:
            ...
            # === 更新状态变化图 ===
            # 清除之前的图形
            for extra_ax in list(self.state_evolution_figure.axes):
                if extra_ax is not self.state_evolution_ax:
                    extra_ax.remove()
            self.state_evolution_ax.clear()  # 添加这行代码
            ...
            action_markers = {
                ...
                2: {'marker': 's', 'color': 'green', 'size': 24, 'label': 'VI+PM'},
                3: {'marker': 'D', 'color': 'blue', 'size': 24, 'label': 'NDT+PM'},
                4: {'marker': '*', 'color': 'purple', 'size': 32, 'label': 'RE'}
            }
            added_labels = set()
            ...
            self.action_ax.set_ylim(0, 21.0)
            self.action_ax.set_yticks([0, 5, 10, 15, 20])
            self.action_ax.set_title("Action Execution Counts", fontname='Times New Roman')
            self.action_ax.set_xticks(range(5))
            self.action_ax.set_xticklabels(["VI", "NDT", "VI+PM", "NDT+PM", "RE"])
            ...
            # 创建图例句柄
            legend_handles = []
            for action_id, marker_info in action_markers.items():
                handle = self.action_ax.scatter([], [],
            ...
            # 添加图例到柱状图
            self.action_ax.legend(handles=legend_handles, loc='upper right', bbox_to_anchor=(1, 1))
    ...
    def update_actor_plot(self, probabilities):
        ...
        # 定义动作标签和颜色
        actions = ["VI", "NDT", "VI+PM", "NDT+PM", "RE"]
        colors = ['#3498db', '#e74c3c', '#2ecc71', '#f1c40f', '#9b59b6']
    ...
    def initUI(self):
        self.setWindowTitle('Infrastructure management System V1.0')
...
class environment():
    """this part define the component degradation process"""
    def __init__(self):
        self.state_number = 5
...
if __name__ == '__main__':
    ...
    if sys.platform == 'win32':
        try:
            ...
            app_id = 'Infrastructure management system.1.0.0'
